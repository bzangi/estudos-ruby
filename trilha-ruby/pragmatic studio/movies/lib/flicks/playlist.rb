  ...
  class Playlist
    ...
    def load(from_file)
      File.readlines(from_file).each do |line|
        add_movie(Flicks::Movie.from_csv(line))
      end
    end
    ...
    def play(viewings)
      ...
      snacks.each do |snack|
        puts "#{snack.name.capitalize} with #{snack.carbs} carbs"
      end

      1.upto(viewings) do |count|
        puts "\nViewing #{count}: "
        @movies.each do |movie|
          Flicks::Reviewer.review(movie)
    ...
    def print_stats
      ...
      @movies.sort.each do |movie|
        ...
        puts "#{movie.carbs_consumed} total carbs"
      end
